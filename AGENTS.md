...
| Resuming earlier work ("上次", "昨天", "继续") | `docs/agents/skills/vibebuddy-history/SKILL.md`: newest handoff first, then the saved summary; a summary never overrides a newer handoff. Optional `status --exclude-session` reports other sessions in this checkout; report them and let the user decide. History tools are read-only |
