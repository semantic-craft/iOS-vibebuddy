...
public final class SoundPolicy {
    ...
    public func evaluate(_ input: SoundPolicyInput) -> [SoundAlert] {
        ...
        for session in input.sessions {
            if let restored = restoredWait(session, input: input) {
                alerts.append(restored)
                if input.now.timeIntervalSince(session.statusSince) >= config.longWaitThreshold {
    ...
    private func rememberPresence(_ input: SoundPolicyInput) {
        let live = Set(input.sessions.map(\.id))
        presenceSuppressedWaits = presenceSuppressedWaits.filter { live.contains($0.key) }
        presenceRestoredWaits = presenceRestoredWaits.filter { live.contains($0.key) }
        for session in input.sessions {
