...
4. **Status hooks stay non-blocking; the daemon is not read-only.** Status hooks return immediately so a missing daemon cannot stall the agent. Remote approval is an opt-in blocking gate. Hook routes share the install bearer token (**ADR-0009**).
