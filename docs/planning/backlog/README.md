...
| A-12 | 公开版关 App 提醒：CloudKit 私有库推送进正式 App | [02](public-push/issues/02-a12-cloudkit-cue-push.md) | **代码完成，开发环境已验收**；发布前还要部署 Production schema 并复测 | Hermes 锁屏、App 已结束：19/19 送达，Mac 开始保存 → 手机收到 p50 1.31 s、p95 1.75 s；手机上点「批准」回到了 Mac，手表那次放行也到了（待你确认是手表点的）；与手机本地通知不重复（ADR-0012 两个方向都成立）。Mac 配了 `.p8` 时 CloudKit 不启用，你现在的用法不变 | 下一次发布（Mac 1.3.35 / iOS 1.3.30）前：CloudKit Console 部署 Production schema，公证版 Mac + Production iPhone 复测一次 |
