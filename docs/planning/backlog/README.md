...
| AI-10 | 转录读取不要覆盖状态行给的上下文窗口和型号 | [10](agent-integration-2026-09/issues/10-transcript-overrides-statusline-window.md) | done（PR 待合并） | 2026-09-24 验收发现：1M 上下文的 Claude 会话在两次状态行之间显示为 200k 窗口，占用被放大约 5 倍。现在状态行报过的型号和窗口不再被转录覆盖，转录只更新 token 数；模型切换后转录可以重新补。回归测试先红后绿；隔离 daemon 快照为 1M / 显示名 / token 40039 | 真实 1M 会话随下次装机复看 |
...
3. ~~**Codex 重新信任**~~ **已完成**（2026-09-24）：你已在 `/hooks` 信任，`vibebuddyd hooks status` 显示「Codex is running all 14 VibeBuddy hooks」，路径是固定目录。剩下的由 agent 做：下次重新部署后确认 Codex 仍视为已信任。
