...
| A-12 前置 | CloudKit 私有库提醒推送原型 | [01](public-push/issues/01-cloudkit-alert-push-prototype.md) | **done**（门槛通过，2026-09-26；票据保留作验收记录） | Hermes 锁屏、App 不在运行：20/20 送达，Mac 开始保存 → 横幅 p50 2.0 s、**p95 3.4 s**（门槛 30 s）；突发 5 条、同会话 3 条都送达；通知扩展在锁屏时取到加密详情并设成时间敏感；锁屏收到的提醒点「批准」经 bearer 回到 Mac（HTTP 200）。手表只显示通知、没有按钮（原型没有手表 App）；Developer ID 包只能用 Production 环境，schema 要先部署。证据在 `~/Projects/_shared-work/iOS-vibebuddy/cloudkit-cue-2026-09-26/` | A-12 按 D 实现；票据 Comments 末尾列了 6 条实现条件（正式容器与 Production schema、就绪判定、账号比对、通用文案、订阅与清理、`.p8` 与 CloudKit 同机只走一路），手表按钮和专注模式在 A-12 验收里补测 |
