...
| WR-12 候选 | 手机锁屏时，新开的任务到不了手表 | —（见 `watch-round-2026-09-25/RESULTS.md`） | 待确认、待开票 | 另一会话 2026-09-25 发现：`WatchStateStore.becameActive()` 只重读 application context，不发 `WatchRefreshRequest`；手机锁屏后流断，锁屏后才开始的任务永远出现不到手表列表上，也就没法从手腕停下。手表一发 `sendMessage` 就能叫醒手机（同一轮里详情页刷新在锁屏时到了 Mac），修法方向是 `becameActive` 时（限频）请求一次刷新 | 与发现它的会话确认谁来修 |
...
1. **手表检查（约 10 分钟，≤ 5 步）**：只验真手腕才能证明的：「回复」进卡片作答（WR-09），且回答在推送后超过 25 s 仍能送到（WR-11）；「返回总览」（WR-10）；另请确认 2026-09-25 15:45 从手表停下时手机是锁着的（WR-08）。步骤由 agent 准备好后单独发给你；Mac 那边由 agent 驱动并读日志。之前已通过：锁屏时横幅「拒绝」「批准」、卡片快捷回答 + 双指互点两下、解锁时停下（2026-09-24），锁屏时停下（2026-09-25）。WR-07 的横幅口述不再验（watchOS 27 上走不到，见 WR-09）。
