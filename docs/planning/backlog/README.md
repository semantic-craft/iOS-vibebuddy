...
仓库状态（2026-09-23 收尾）：没有开放 PR；远端只剩 `main` 与 `gh-pages`；只剩一个 worktree（本会话）。相关会话已归档。已删除测试残留：`$TMPDIR` 下约 6500 个 `vb-*` / `recap-*` 等测试临时目录、约 40 个 QA / E2E 用的 `com.vibebuddy.*` 偏好域（保留正式的 `com.vibebuddy.mac`）。
