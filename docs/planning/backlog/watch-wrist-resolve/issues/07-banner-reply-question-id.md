...
- 2026-09-24 腕上验收（Hermes 开发版 1.3.28 (58)，`main` 9137f92f；手表 Series 10，watchOS 27；Mac 1.3.32 开发版，:9876）：**横幅口述这条路径在这块表上走不到。** 点横幅「回复」后 5–9 s，诊断记为 `notification.action-opens`（`userText` 为空），打开的是 App 自己的卡片，不是系统输入框。什么都没发出去，是安全的，但真机勾选项第 1 条（横幅口述答到那一题）和第 3 条（换题拒绝、句子留在卡片）都无法按原设计验收。转到 [09](09-banner-reply-opens-app.md) 定方案。走卡片回答正常：点横幅文字 → 卡片 → 预设「Yes」+ 双指互点两下，agent 收到的正是那一题（推送后 20 s）。绑定逻辑本身已在模拟器上验过（见上一条）。
