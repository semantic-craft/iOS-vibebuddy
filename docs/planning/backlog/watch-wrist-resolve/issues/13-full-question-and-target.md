...
5. **结果片段**：`WatchFollowedTask` 新增可选字段 `resultExcerpt`，取 Mac 已发给手机的 `completionText`（只取已完成、有 `completionID` 的这一轮，本地再限 280 字；不进表盘快照）。详情页依次显示「Mac 生成的完成摘要」和「agent 回复的开头」，各自一段，超过约 4 行（`resultLimit` = 110）就给「展开」。都没有时回落到原来的状态摘要 / 「结果请在手机上看」。
