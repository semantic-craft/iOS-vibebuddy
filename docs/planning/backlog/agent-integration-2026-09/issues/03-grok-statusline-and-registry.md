...
- 名册：TUI 启动即登记自身 pid；正常退出（SIGHUP）清空；`kill -9` 留下死 pid 的条目，直到下一个 grok 写文件。正常关闭时 `SessionEnd` hook 约 1 s 移除行；`kill -9`（hook 不可能触发）后 16 s，下一次 sweep 由名册兜底结束该行（日志 `grokSessionClosed`，来源 `recovery`）。
- 只结束"见过在名册里、后来不在或进程已死"的会话；名册关闭、别的 `GROK_HOME`、重启后恢复的行都不会被误结束。
