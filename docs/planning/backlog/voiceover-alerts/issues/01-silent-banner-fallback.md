...
- 回归测试在路由这一层：VoiceOver 开 + 横幅不会出现 → 卡片 + 念出 + 声音，不发横幅；VoiceOver 开 + 横幅会出现 → 只发横幅；VoiceOver 开 + 横幅不会出现 + 刘海隐藏 → 进通知中心 + 念出；只进列表的提醒 → 卡片 + 声音，不念。把路由临时改回旧逻辑，卡片和刘海隐藏两条测试失败（实测；列表那条当时还没有，按代码推理也会失败）；去掉"只念打扰级别"的判断，列表那条失败。
...
- 2026-09-26 第 2 轮：MERGE。上轮两条均 FIXED，调整顺序无回归；只改了本票「验证」里失败测试条数的措辞。
