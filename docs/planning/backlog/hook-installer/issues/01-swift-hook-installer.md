...
- 2026-09-24（PERF-02 会话）：信任后的重新部署检查。11:00Z 从 main 43c52e0f 替换 `/Applications` 后，`vibebuddyd hooks status` 显示「Codex is running all 14 VibeBuddy hooks」；12:08Z 另一个会话又从 main 替换了一次（含 #301），之后再查，结果不变（两次的输出都存在 `~/Projects/_shared-work/iOS-vibebuddy/perf-02-2026-09-24/hooks-status-after-*.txt`）。命令字符串没变，Codex 没有要求重新信任，第 41 行已勾选。
