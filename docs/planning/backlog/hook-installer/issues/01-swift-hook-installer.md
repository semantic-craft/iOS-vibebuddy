...
- 2026-09-24：本机 Codex 已迁移。按设置页「修复」同样的调用跑了 `vibebuddyd hooks install --agent codex`，脚本来自 `/Applications` 的 bundle。`~/.codex/hooks.json` 的 14 条命令全部改指 `~/Library/Application Support/vibebuddy/bin/`，`config.toml` 没变。备份在 `~/Projects/_shared-work/iOS-vibebuddy/acceptance-2026-09-24/03-codex-migration/backup/` 和安装器自己的备份目录。迁移后 `hooks status` 报告 Codex「skipping 14 of 14」：信任之前 Codex 的 hook 全部不运行。待办：owner 用 `/hooks` 信任；信任之后的下一次重新部署时，再确认 Codex 仍视 hook 为已信任。
