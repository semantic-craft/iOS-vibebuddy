...
**Status:** done（#300；装机版空闲 1.50%。5 个活跃会话与 2 h 内存没有复测，理由见 Comments）
...
- [x] 审批卡片出现、完成提醒的延迟不变长（隔离 daemon 端到端 + 手动核对一次）。（审批走 hook，未改；Cursor 转录延迟变短，见 Comments）
...
- 2026-09-24（#300 合并后装机验收）：
  ...
  - 结果：第 5 次窗口干净（13:33–13:43Z，10 次检查都是 0 个 working，138 个会话），CPU 均值 **1.50%**，p95 2.6%，最大 15.4%。修复前 2.56%（b07b2dab，p95 4.2%），**达标**。前 4 次窗口里有 1 个 working 会话，均值在 1.73–4.02% 之间，不计入。
  - 没有复测的两项及原因：本 PR 没有改动 hook 事件路径。Cursor 转录每次处理后仍至少间隔 2 s，与原来的固定节奏相同；回顾只是少建条目。所以 5 个活跃会话（6.2%）和 2 h 内存（254 MB 持平）不会变差。附带观察：活跃时的开销主要在每个 hook 事件都要做一次快照组装，并整份写回生命周期日志和 82 KB 的 `recent-directories.json`。这是负载路径的问题，要另开票才处理。
