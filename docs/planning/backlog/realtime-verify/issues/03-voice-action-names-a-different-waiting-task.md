...
- [x] 评审后重跑（`round2/`）。评审发现两处缺陷：一是 Qwen 的转写晚到时，上一句的名字会放行这一句的动作；二是 GPT-Live 的连续音频把用户字幕切碎。改成：用户一开口（`speechStarted`）就重新计；Live 只按字幕分组；非 final 转写一律当作「这一句到目前为止」，后一条替换前一条（Gemini 适配层改成自己按句累积，原来是一段段增量）。重跑结果：Qwen 中文 3/3、Gemini 英文 3/3，「不点名 → 说出名字」Qwen 和 Gemini 英文各 1/1，都落对了。Gemini 中文那一轮把用户说的「橘子」转写成了日文「じゅ ず」，模型本身听对了，但动作被扣住，属于安全方向的误扣，也就是上面说的代价。并发跑 5 路时有 2 路在第 3 步卡住超时（音频在超时后才送完），单独重跑都正常；另有一次 Gemini 连接超时，都是环境问题。整个过程中没有一次动作落到用户没点名的任务上。
- 已知残留缺口，已接受并写进 ADR-0008：Gemini 只在被打断时才发「开始说话」事件，所以如果它的工具调用比新一句的第一段转写先到，上一句的词还算数。GPT-Live 的字幕同理，要停顿超过 1.5 s 才会另起一组。回放里转写每次都先到。
