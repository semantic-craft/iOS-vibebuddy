...
- `cd VibeBuddyKit && swift test`: 584 Swift Testing tests in 90 suites and 85 XCTest tests pass, including `ProviderLimitSignalTests` and the new `VoiceCallCoordinatorTests` cases (limit ending, unrelated endings, send failure at the cap, zh-Hans notice).
