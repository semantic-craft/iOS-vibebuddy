...
- iPhone: the voice strip and voice page show the notice with a **Redial** button; Mac: the Voice and reading panel shows it with **Redial**, the dashboard sidebar's Voice row shows the notice. `VoiceChat.redial()` starts an ordinary new call from the current Settings; nothing from the ended call is sent.
- `cd VibeBuddyKit && swift test`: 584 Swift Testing tests in 90 suites and 85 XCTest tests pass, including `ProviderLimitSignalTests` and the new `VoiceCallCoordinatorTests` cases (limit ending, unrelated endings, fresh redial, zh-Hans notice).
