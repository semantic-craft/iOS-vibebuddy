...
评审第一轮提了 9 处修改（签名描述文件检查、iCloud 状态移出轮询、订阅建成才报就绪、清理时机、隐私政策措辞等），全部改完。第三轮结论为 MERGE。

改完后在 Hermes 上复测：手机锁屏，新装的 App 还没运行过，3 条都送达，通知扩展取回了加密详情。从 Mac 开始保存到扩展收到，分别为 0.94 s、0.74 s、1.17 s。
