...
| W2 | high | At large text sizes the question (3 lines), the command under approval (4 lines), the question-walk text (4 lines) and each answer option (2 lines, scaled to 70 %) were cut: the wearer could approve or pick something they had not read in full. | Limits lift at accessibility sizes; options and quick replies never truncate. | AX5 tree: the command's frame grows from 68 to 84.5 pt (whole command shown). |
