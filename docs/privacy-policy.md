...
- **iCloud notifications:** when your Mac has no push key of its own, it reaches your iPhone through your own iCloud account instead. The Mac saves a short notification record to the private iCloud database of the Apple Account you are signed into, and Apple's iCloud servers send the alert to your iPhone. The notification's title and text are stored end-to-end encrypted, so the alert Apple delivers carries only generic text such as "An agent is waiting for your approval". Stored in the clear, and so visible to Apple, are only identifiers and a time: the session and request identifiers the notification belongs to, the kind of notification, and when it was sent. While the Mac app is running, records are deleted within about fifteen minutes of being sent; any left behind when it quits are deleted the next time it runs. The iPhone also tells your Mac its iCloud user identifier for this app, so the Mac can check that both devices use the same Apple Account. VibeBuddy operates no server in this path and cannot read your iCloud data.
...
- We do not sell your data. Enabled notifications use Apple's push service (or your own iCloud private database, as above), and optional model and speech requests go directly to your configured providers. VibeBuddy operates neither service and receives neither payload.
