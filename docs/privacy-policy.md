...
- **iCloud notifications:** when your Mac has no push key of its own, it reaches your iPhone through your own iCloud account instead. The Mac saves a short notification record to the private iCloud database of the Apple Account you are signed into, and Apple's iCloud servers send the alert to your iPhone. The notification's title and text are stored end-to-end encrypted, so Apple sees only generic text such as "An agent is waiting for your approval". Records are deleted about ten minutes after they are sent. The iPhone also tells your Mac its iCloud user identifier for this app, so the Mac can check that both devices use the same Apple Account. VibeBuddy operates no server in this path and cannot read your iCloud data.
