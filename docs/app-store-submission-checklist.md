...
现状:本机只有免费级 "Apple Development" 证书;iOS `aps-environment=development`;version 0.1;有 1024 图标 ✓。
...
- [ ] B1. 🤖 **`aps-environment` 改 production**(VibeBuddyApp.entitlements)—— App Store/TestFlight 必须。
...
- [ ] B4. 🤖 核 `VibeBuddyApp.entitlements`:Push ✓;若 Widget/Live Activity 与主 app 共享数据,需要 **App Group**(待核)。
- [ ] B5. 🤖 版本号 `MARKETING_VERSION` 0.1 → **1.0**(首发),build = 1。
- [ ] B6. 🤖 加 `ITSAppUsesNonExemptEncryption = NO`(只用标准 HTTPS),省得每次问 export compliance。
- [ ] B7. 图标:1024 marketing icon 已有 ✓(无需补)。
...
- [ ] C5. **截图**:至少一组 6.7"(或 6.9")iPhone —— 看板+吉祥物、审批卡 diff 各一张。🧑 模拟器截。
...
- [ ] E3. **App Review Information(伴侣 app 关键)**:写清楚这是开源 Mac 工具 vibebuddy 的手机端、如何配对(扫 Mac 二维码 / 同一 LAN);**附 demo 视频 + 详细步骤**,因为审核员没有你的 Mac。🤖 我起草 reviewer notes(英文)。
