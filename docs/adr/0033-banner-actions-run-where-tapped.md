...
Hermes on a development build of main 9137f92f, Apple Watch Series 10 on watchOS 27, phone locked and face down, fresh notifications.
...
- **Reply from the wrist banner: never reached the watch with text.** watchOS opened the app with `userText` empty (`notification.action-opens`), 5–9 s after the push. Decision 5's binding is proven in the simulator only. What the button should become is ticket watch-wrist-resolve 09.
