...
- 「在外连接」改为以官方 Tailscale 为主：直接给出安装、登录步骤，并显示这台 iPhone 是否已接入 tailnet；Headscale 和 Surge 照旧可用，两者都装时可以选。修正了蜂窝网络被误判为已接入 tailnet 的情况，以及设置步骤第一次打开时文字重叠的问题。
...
- Connect away from home now leads with official Tailscale: install and sign-in steps, plus whether this iPhone is on your tailnet. Headscale and Surge still work, with a choice when both are installed. Fixed cellular networks being mistaken for a tailnet connection, and overlapping text the first time the setup steps opened.

iOS build 63 (replaces build 62, withdrawn from review before it was processed). The accompanying Mac release is 1.3.35 (53).
