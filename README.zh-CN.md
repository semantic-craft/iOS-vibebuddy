...
| **腕上通知直达任务** | 点击完成通知可打开对应详情，并在 iPhone 锁屏时请求最新结果；刷新失败会标示缓存并提供重试。各端的回顾（Recap）已移除，保留任务列表和明确的已读操作。[Watch 更新](docs/release-notes-1.3.17.md) |
...
| **语音、摘要与朗读** | 可选语音对话、完成摘要和 Mac 朗读分别配置，支持 OpenAI、千问和豆包等已接入服务商。[语音设置](docs/release-notes-1.3.11.md) |
...
选择 **OpenAI、阿里千问或火山引擎豆包**，在 App 内填写自己的 API 凭据、阅读告知后主动开始通话。可以询问“哪个任务需要我？”，也可以明确回答当前可处理的请求。实时对话、完成摘要和 Mac 朗读分别配置。服务商的单次通话时长到顶时，App 会说明通话已结束并提供一键重拨；新通话不带之前的对话内容。服务商可能收费；任务看板和按钮审批不需要 AI API 密钥。
