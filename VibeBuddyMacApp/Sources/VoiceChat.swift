...
/// The Mac voice companion: tap the buddy to hold a realtime speech-to-speech
/// conversation with the selected provider. Provider sessions and shared call
/// state live in `VibeBuddyKit`; this controller supplies Mac audio I/O and UI
...
final class VoiceChat: ObservableObject {
    ...
    /// Runs the microphone TCC prompt. `nonisolated` keeps the SDK callback off
    /// the main actor, then the caller hops back to update `@Published` state.
    nonisolated private static func requestMic(_ done: @escaping @Sendable (_ mic: Bool) -> Void) {
