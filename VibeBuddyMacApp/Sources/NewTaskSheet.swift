...
/// has already run in, write the prompt, optionally name it. Codex starts
/// through the app-server daemon and appears in Desktop too; other agents are
/// listed but disabled until they have a launcher.
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Picker("Agent", selection: $agent) {
                ...
                Text("Claude Code (soon)").tag(AgentKind.claudeCode)
            }
            ...
            HStack {
                ...
                Button(busy ? "Starting…" : "Start") { start() }
                    ...
                    .disabled(busy || agent != .codex || directory.isEmpty
                              || prompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
            }
        }
        .padding(20)
        .frame(width: 520)
        .onAppear { if directory.isEmpty { directory = model.recentDirectories.first ?? "" } }
    }
