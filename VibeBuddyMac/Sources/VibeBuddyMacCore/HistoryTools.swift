...
public enum HistoryToolError: Error, LocalizedError {
    case readOnly, noIndex, refreshBusy, invalidArguments(String), invalidValue(String), executionFailed(String)
...
public enum HistoryTools {
    public static func definitions() -> [[String: Any]] {
        [definition("vibebuddy_get_session", description: "Read a transcript by native key or reference; sequence positions are stable for its source revision.", properties: [
            ...
            "since": ["type": "string"], "limit": ["type": "integer", "minimum": 1, "maximum": 200]
        ], required: ["query"])] + [HistoryLiveStatus.definition, HandoffFacts.definition]
    }
    ...
    public static func call(_ name: String, arguments: [String: Any], snapshot: SessionHistorySnapshot, now: Date = Date()) throws -> String {
        guard !["vibebuddy_get_session", "vibebuddy_search", "vibebuddy_get_summary", "vibebuddy_live_status", HandoffFacts.toolName].contains(name) else {
...
public enum HistoryCLI {
    public static let commands = ["sessions": "vibebuddy_list_sessions", "projects": "vibebuddy_list_projects", "show": "vibebuddy_get_session", "search": "vibebuddy_search", "summary": "vibebuddy_get_summary", "status": "vibebuddy_live_status", "facts": HandoffFacts.toolName]
    public static func parse(_ argv: [String]) throws -> (tool: String, arguments: [String: Any]) {
        ...
        if ["show", "search", "summary", "facts"].contains(command) {
            ...
            args[command == "search" ? "query" : "key"] = argv[1]; index = 2
        }
        while index < argv.count {
            let flag = argv[index]
            if ["--starred", "--tools", "--thinking"].contains(flag) { args[String(flag.dropFirst(2))] = true; index += 1; continue }
            guard ["--project", "--agent", "--since", "--limit", "--from-seq", "--max-messages", "--exclude-session", "--commands", "--cwd"].contains(flag), index + 1 < argv.count else {
