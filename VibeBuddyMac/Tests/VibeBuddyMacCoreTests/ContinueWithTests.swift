...
final class ContinueWithTests: XCTestCase {
    ...
    @MainActor
    func testRecordedContinuationReachesTheSnapshotAndTheHandoffRecord() async throws {
        ...
        await store.recordContinuation(sessionID: "new", sourceKey: "claude-code:src", handoffPath: path.standardizedFileURL.path)
        let after = await store.snapshot(now: now.addingTimeInterval(3))
        XCTAssertEqual(after.handoffs?.first?.takenBy, ["new"])
